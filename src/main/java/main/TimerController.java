...
public class TimerController implements Initializable {
        ...
        try {
            if (txtcustomtime == null || txtcustomtime.getText().isEmpty()) {
                ...
                if (Utils.isNumeric(combotime.getValue().substring(2, 3))) {
                    ...
                    time = Integer.parseInt(combotime.getValue().substring(0, 3));
                    time = TimeUnit.MINUTES.toMillis(time);
                } else if (Utils.isNumeric(combotime.getValue().substring(1, 2))) {
                    time = Integer.parseInt(combotime.getValue().substring(0, 2));
                    time = TimeUnit.MINUTES.toMillis(time);
                } else {
                    time = Integer.parseInt(combotime.getValue().substring(0, 1));
                    time = TimeUnit.MINUTES.toMillis(time);
                }
            } else {
                time = Integer.parseInt(txtcustomtime.getText());
                time = TimeUnit.MINUTES.toMillis(time);
            }
    ...
    /**
     * Cancels the action main before it's fired.
     *
     * @param event
     */
    @FXML
